...
@st.cache_data(ttl=600) # Re-check the sheet every 10 mins
def load_reference_data(url):
    # DUMMY DATA FALLBACK (If you haven't pasted the link yet)
    if "YOUR_LONG_ID" in url:
        return pd.DataFrame([
            {"Ticker": "BHP", "Expiry": "2026-02-19", "Strike": 50.00, "Type": "Call", "Code": "BHPS49"},
            {"Ticker": "BHP", "Expiry": "2026-02-19", "Strike": 50.65, "Type": "Put", "Code": "BHPW82"}
        ])
    ...
    try:
        ...
        df['Type'] = df['Type'].astype(str).str.title().str.strip() # "Call" or "Put"
        df['Strike'] = pd.to_numeric(df['Strike'], errors='coerce')
        
        # Date parsing can be tricky. We try standard formats.
...
def lookup_code(ticker, expiry_str, strike, kind):
    # expiry_str from Yahoo is '2026-02-19'
    ...
    try:
        ...
        mask = (
            ...
            (df['Type'] == kind) &
            (np.isclose(df['Strike'], strike, atol=0.01)) & # Handles 50.0 vs 50.00
        ...
        return "ENTER_CODE" # Not found in sheet
    except:
        return "ERROR"
