...
def get_bs_price(kind, spot, strike, time_days, vol_pct, rate_pct=4.0):
    try:
        ...
        # Enforce minimum time (15 mins) to prevent math errors
...
if st.session_state.ref_data is not None:
    ...
    if not subset.empty:
        # Filter Future Expiries (Prevent Delta 0 on expired chains)
        ...
        if valid_exps:
            ...
            st.warning("No future option expiries found.")
